...
export default function SettingsView() {
  ...
  useEffect(() => {
    // Fetch the current user to display their email
    const getUser = async () => {
      ...
      <div className="w-full max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-8">
          ...
          <form onSubmit={handleUpdatePassword} className="space-y-4">
            ...
            {/* Status Messages */}
            {message && (
              <div className={`px-4 py-3 rounded-xl text-sm font-bold border flex items-center gap-2 ${message.type === 'error' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
