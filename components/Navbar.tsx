...
export default function Navbar() {
  ...
  const [userProfile, setUserProfile] = useState<any>(null); // State for the public profile
    ...
    const initNav = async () => {
      ...
      if (user) {
        ...
        const channel = supabase.channel(`nav-notifications-${user.id}`)
          ...
          // Listen to the profiles table so the navbar instantly updates if changed in settings!
  ...
  // Safe fallback to initials if the profile image is missing or broken
  const fallbackInitial = userProfile?.email?.charAt(0)?.toUpperCase() || currentUser?.email?.charAt(0)?.toUpperCase() || '?';
